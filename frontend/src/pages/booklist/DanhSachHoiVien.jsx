...
export default function DanhSachHoiVien() {
  const [trang_hien_tai, dat_trang_hien_tai] = useState(1);

  const { data: ket_qua, isLoading: dang_tai } = useQuery({
    queryKey: ["sach_hoi_vien", trang_hien_tai, KICH_THUOC_TRANG],
    queryFn: async () => {
      const phan_hoi = await api.get("/home/sach_hoi_vien", {
  ...
  return (
    <div className="trang_danh_sach">
      <div className="dau_trang_danh_sach">
        ...
        <h1 className="tieu_de_danh_sach">Sách hội viên</h1>
        {tong_ban_ghi > 0 && (
