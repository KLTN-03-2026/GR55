...
export default function QuanLySach() {
  ...
  const tai_danh_sach = useCallback(
    async (trang, tu_khoa, ma_dm, trang_thai) => {
      dat_dang_tai(true);
      try {
  ...
  function kiem_tra_truong(ten, gia_tri) {
    switch (ten) {
      case "ten_sach":
        ...
        if (gia_tri.trim().length > 255)
          return "Tên sách không được vượt quá 255 ký tự";
        return "";
      case "tac_gia":
        if (!gia_tri.trim()) return "Tác giả không được để trống";
        if (gia_tri.trim().length > 100)
          return "Tác giả không được vượt quá 100 ký tự";
        return "";
      case "gia":
        if (gia_tri === "" || gia_tri == null) return "Giá không được để trống";
        if (isNaN(Number(gia_tri)) || Number(gia_tri) < 0)
  ...
  function xu_ly_chon_anh(e) {
    ...
    if (!tap_tin.type.startsWith("image/")) {
      dat_loi_form((prev) => ({
        ...prev,
        anh_bia: "Vui lòng chọn file ảnh (JPG, PNG, ...)",
      }));
      return;
    }
    if (tap_tin.size > 4 * 1024 * 1024) {
      dat_loi_form((prev) => ({
        ...prev,
        anh_bia: "Ảnh bìa không được vượt quá 4MB",
      }));
      return;
    }
  ...
  function xu_ly_chon_pdf(e) {
    ...
    if (tap_tin.size > 100 * 1024 * 1024) {
      dat_loi_form((prev) => ({
        ...prev,
        file_pdf: "File PDF không được vượt quá 100MB",
      }));
      return;
    }
  ...
  async function xu_ly_luu() {
    ...
    try {
      ...
      form_data.append(
        "du_lieu",
        new Blob([du_lieu_json], { type: "application/json" }),
      );
      ...
      const thong_bao =
        loi_api.response?.data?.message || "Có lỗi xảy ra. Vui lòng thử lại.";
  ...
  return (
    <div className="trang_quan_ly">
                ...
                {dang_tai ? (
                  ...
                  danh_sach.map((sach) => (
                    <tr key={sach.ma_sach}>
                      ...
                      <td>
                        <span className="ten_sach_bang" title={sach.ten_sach}>
                          {sach.ten_sach}
                        </span>
                      </td>
                      <td>{sach.tac_gia}</td>
                      <td>
                        <div className="nhom_nhan_danh_muc">
                          {(sach.ten_danh_muc || [])
                            .slice(0, 2)
                            .map((ten, i) => (
                              <span key={i} className="nhan_danh_muc">
                                {ten}
                              </span>
                            ))}
                          {(sach.ten_danh_muc || []).length > 2 && (
                            <span className="nhan_danh_muc_them">
                              +{sach.ten_danh_muc.length - 2}
                            </span>
                          )}
                        </div>
                      ...
                      <td>
                        {Number(sach.gia) === 0 ? (
                          <span className="nhan_trang_thai mien_phi">
                            Miễn phí
                          </span>
                        ) : (
                          <span className="nhan_trang_thai tra_phi">
                            Trả phí
                          </span>
                        )}
                      </td>
                      <td>
                        <div className="nhom_nut_thao_tac">
                          <button
                            className="nut_sua"
                            onClick={() => mo_modal_sua(sach)}
                          >
                            Sửa
                          </button>
                          <button
                            className="nut_xoa"
                            onClick={() => mo_modal_xoa(sach)}
                          >
                            Xóa
                          </button>
                        </div>
      ...
      {hien_modal_form && (
        <div
          className="nen_modal"
          onClick={(e) => e.target === e.currentTarget && dong_modal_form()}
        >
            ...
            <div className="luoi_form_sach">
              ...
              <div className="cot_form">
                ...
                <div className="nhom_truong_modal">
                  <label className="nhan_truong_modal">
                    Tên sách <span className="dau_bat_buoc">*</span>
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) => {
                      ...
                      dat_loi_form((prev) => ({
                        ...prev,
                        ten_sach: kiem_tra_truong("ten_sach", e.target.value),
                      }));
                    }}
                    placeholder="Nhập tên sách"
                    maxLength={255}
                  />
                  <span className="thong_bao_loi_modal">
                    {loi_form.ten_sach}
                  </span>
                </div>
                ...
                <div className="nhom_truong_modal">
                  <label className="nhan_truong_modal">
                    Tác giả <span className="dau_bat_buoc">*</span>
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={(e) => {
                      ...
                      dat_loi_form((prev) => ({
                        ...prev,
                        tac_gia: kiem_tra_truong("tac_gia", e.target.value),
                      }));
                    }}
                ...
                <div className="nhom_truong_modal">
                  <label className="nhan_truong_modal">
                    Giá (VNĐ) <span className="dau_bat_buoc">*</span>
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={(e) => {
                      ...
                      dat_loi_form((prev) => ({
                        ...prev,
                        gia: kiem_tra_truong("gia", e.target.value),
                      }));
                    }}
                ...
                <div className="nhom_truong_modal nhom_doc_thu">
                  <label className="nhan_checkbox">
                    <input
                      ...
                      onChange={(e) =>
                        cap_nhat_truong("cho_phep_doc_thu", e.target.checked)
                      }
                    />
                  ...
                  {gia_tri_form.cho_phep_doc_thu && (
                    <div className="nhom_so_trang_doc_thu">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          cap_nhat_truong("so_trang_doc_thu", e.target.value)
                        }
                        min={1}
                        max={999}
                      />
              ...
              <div className="cot_form">
                ...
                <div className="nhom_truong_modal">
                  <label className="nhan_truong_modal">
                    Ảnh bìa{" "}
                    {!dang_sua && <span className="dau_bat_buoc">*</span>}
                    {dang_sua && (
                      <span className="chu_thich_tuy_chon">
                        {" "}
                  ...
                  <div
                    className="khung_chon_tap_tin"
                    onClick={() => ref_o_nhap_anh.current?.click()}
                  >
                    {xem_truoc_anh ? (
                      <img
                        src={xem_truoc_anh}
                        alt="Xem trước"
                        className="xem_truoc_anh_bia"
                      />
                    ) : (
                      <div className="goi_y_chon_tap_tin">
                ...
                <div className="nhom_truong_modal">
                  ...
                  >
                    {tap_tin_pdf ? (
                      ...
                      <span className="goi_y_doi_tap_tin">
                        Nhấn để thay thế file PDF
                      </span>
                    ) : (
                      <span className="goi_y_chon_tap_tin">
                ...
                <div className="nhom_truong_modal">
                  ...
                  <div className="luoi_chon_danh_muc">
                    ...
                    {tat_ca_danh_muc.length === 0 && (
                      <span className="khong_co_danh_muc">
      ...
      {sach_xoa && (
        <div
          className="nen_modal"
          onClick={(e) => e.target === e.currentTarget && dong_modal_xoa()}
        >
          <div className="hop_modal">
            ...
            {loi_server_xoa ? (
              ...
              <p className="noi_dung_modal_xoa">
                ...
                <span className="ten_xoa_noi_bat">"{sach_xoa.ten_sach}"</span>{" "}
                không?
              </p>
            )}
            <div className="nhom_nut_modal">
              {loi_server_xoa ? (
                ...
                <>
                  <button
                    className="nut_huy"
                    onClick={dong_modal_xoa}
                    disabled={dang_xoa}
                  >
                    Không
                  </button>
                  <button
                    className="nut_xoa_xac_nhan"
                    onClick={xu_ly_xoa}
                    disabled={dang_xoa}
                  >
